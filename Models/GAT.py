...
class GAT(torch.nn.Module):
    ...
    def __init__(self, num_of_layers, num_heads_per_layer, num_features_per_layer, num_nodes, add_skip_connection=True, bias=True,
...
class GATLayer(torch.nn.Module):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, num_nodes, layer_type, concat=True, activation=nn.ELU(),
...
class GATLayerImp3(GATLayer):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, num_nodes, concat=True, activation=nn.ELU(),
                 dropout_prob=0.6, add_skip_connection=True, bias=True, log_attention_weights=False):

        # Delegate initialization to the base class
        super().__init__(num_in_features, num_out_features, num_of_heads, num_nodes, LayerType.IMP3, concat, activation, dropout_prob,
    ...
    def forward(self, data):
        ...
        in_nodes_features, edge_index, attention_layer = data  # unpack data
        ...
        attentions_per_head = None
        if self.num_of_heads == 1 :
            attentions_per_head = torch.tensor(attentions_per_edge.permute(1, 0, 2).squeeze(2).contiguous().view(self.num_of_heads, num_of_nodes, num_of_nodes))
            attentions_per_head = attentions_per_head.squeeze(0)  
            #attentions_per_head = torch.mm(attentions_per_head, self.context_vector)
            #attentions_per_head = attentions_per_head.squeeze(1)
        ...
        return (out_nodes_features, edge_index, attentions_per_head)
...
class GATLayerImp2(GATLayer):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, num_nodes, concat=True, activation=nn.ELU(),
        ...
        super().__init__(num_in_features, num_out_features, num_of_heads, num_nodes, LayerType.IMP2, concat, activation, dropout_prob,
                         add_skip_connection, bias, log_attention_weights)
...
class GATLayerImp1(GATLayer):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, num_nodes, concat=True, activation=nn.ELU(),
        ...
        super().__init__(num_in_features, num_out_features, num_of_heads, num_nodes, LayerType.IMP1, concat, activation, dropout_prob,
                         add_skip_connection, bias, log_attention_weights)
...
class BiGAT_RNN(BiRNN):
    def __init__(self,args,embeddings,return_att):
        ...
        self.CORA_NUM_CLASSES = self.hidden_size * 2
    ...
    def forward(self, input_ids=None,attention_mask=None,attention_vals=None,labels=None,device=None):
        ...
        out_put = torch.zeros(h_seq.shape[0], self.CORA_NUM_INPUT, self.CORA_NUM_CLASSES)
        attention = torch.zeros(h_seq.shape[0], self.CORA_NUM_INPUT)
        for i in range(h_seq.shape[0]):
            ...
            #node_features =   torch.arange(self.CORA_NUM_INPUT * self.CORA_NUM_INPUT_FEATURES, dtype=torch.float32).view(self.CORA_NUM_INPUT, self.CORA_NUM_INPUT_FEATURES)
