...
def select_model(params,embeddings):
    if(params['bert_tokens']):
        if(params['what_bert']=='weighted'):
            model = BiAtt_BertModels.from_pretrained(
        ...
        if(text=="birnn"):
            ...
            #model=MEDICAL_BiAtt_RNN(params,embeddings,return_att=True)
            model=BiAtt_RNN(params,embeddings,return_att=True)
            #model=BiCoAtt_RNN(params,embeddings,return_att=True)
            #model=BiGAT_RNN(params,embeddings,return_att=True)
            #model=BiStackAtt_RNN(params,embeddings,return_att=True)
        elif(text == "cnn_gru"):
...
def get_final_dict_with_rational(params,test_data=None,topk=5):
    ...
    ex_c = 0
    ex_s = 0
    ele={"true_comprehensiveness" : 0, "true_sufficiency" : 0, "true_explicabilty_and_true_classification" : 0, "false_explicabilty_and_true_classification" : 0, "true_explicabilty_and_false_classification" : 0, "false_explicabilty_and_false_classification" : 0}
    for ele1,ele2,ele3 in zip(list_dict_org,list_dict_with_rational,list_dict_without_rational):
        if ele3['classification_scores']['normal'] >= ele3['classification_scores']['offensive'] and ele3['classification_scores']['normal'] >= ele3['classification_scores']['hatespeech']:   
            ex_c = 1
            ele["true_comprehensiveness"] += 1
        if ele2['classification_scores']['hatespeech'] >= ele2['classification_scores']['offensive'] :
            tp = 'hatespeech'
        else:
            tp = 'offensive'
        if ele2['classification_scores'][tp] <= ele2['classification_scores']['normal'] :
            tp = 'normal'

        if tp == ele1["rationales"][0]["truth"] :
            ex_s = 1
            ele["true_sufficiency"] += 1
        
        if ele1["classification"] == ele1["rationales"][0]["truth"]:
            cl = 1

        if cl == 1 and (ex_c ==1 and ex_s == 1) :
            ele["true_explicabilty_and_true_classification"] += 1
        else:
            if cl == 1 and (ex_c == 0 or ex_s == 0) :
                   ele["false_explicabilty_and_true_classification"] += 1
            else:
                if cl == 0 and (ex_c == 1 and ex_s == 1) :
                    ele["true_explicabilty_and_false_classification"] += 1
                else:
                    if cl == 0 and (ex_c == 0 or ex_s == 0) :
                        ele["false_explicabilty_and_false_classification"] += 1
        cl = 0
        ex_c = 0
        ex_s = 0
